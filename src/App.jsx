...
export default function App() {
  ...
  return (
    <div className="min-h-screen">
      ...
      <Footer />
      {/* ponytail: invisible img counts the visit on every load; no CORS/backend needed */}
      <img
        src="https://api.visitorbadge.io/api/visitors?path=portoflio-filmark.hero"
        alt=""
        aria-hidden="true"
      ...
      <button
        ...
        className="fixed bottom-3 left-3 z-50 h-2 w-2 cursor-pointer rounded-full bg-(--fg) opacity-15 transition-opacity hover:opacity-60"
      />
      {showVisits && (
        <img
          src="https://api.visitorbadge.io/api/visitors?path=portoflio-filmark.hero"
          alt="Visitor count"
          className="fixed bottom-6 left-6 z-50 rounded"
        />
      )}
    </div>
  )
